...
def orcid_to_address(orcid_id):
    response = requests.get(
        "https://pub.orcid.org/v3.0/" + str(orcid_id) + str("/employments"),
        headers=headers,
    )  # , params=params)
    try:
        temp = response.json()
        address = temp["affiliation-group"][0]["summaries"][0]["employment-summary"][
            "organization"
        ]
...
def name_to_orcid_to_address(NAME):
    ...
    if len(temp):
        ...
        temp = response.json()
        try:
            org_name = temp["affiliation-group"][0]["summaries"][0][
                "employment-summary"
            ]["organization"]["name"]
            org_address = temp["affiliation-group"][0]["summaries"][0][
                "employment-summary"
...
def get_coords():
    orcid_affiliations = {
        k: v for k, v in orcid_affiliations.items() if type(v) is not type(dict())
    }
